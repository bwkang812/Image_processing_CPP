...
public:
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------------------
	//채널 삽입 함수.
	void PutChannelImg(const CMyImage<T>& src, int nChannel) //회색조 영상+ 채널 번호 넣으면 영상 채널을 입력한 회색조 영상으로 대체한다. 
	{
		...
		for (int r=0 ; r<nHeight ; r++)
		{
			...
			{
				pDst[idx] = pSrc[c];        //체널 nChannel에 입력 영상의 픽셀 값 대입. 
				idx += m_nChannels;
			}
		}
	}
	//---------------------------------------------------------------------------
	//채널 추출 함수. 
	CMyImage<T> GetChannelImg(int nChannel) const  //채널번호를 입력하면, 해당 채널을 단일 채널 영상(회색조 영상)으로 반환한다.
	{
		...
		for (int r=0 ; r<m_nHeight ; r++)
		{
			...
			for (int c=0 ; c<m_nWidth ; c++)
			{
				pDst[c] = pSrc[idx];  //결과 영상에 채널 nChannel에 해당하는 픽셀 값 대입. 
				idx += m_nChannels;
			}
		}
	...
	CMyImage<T> PEPSI( ) const //펩시콜라인 부분을 1 ,아닌 부분을 0으로 바이너리 한다. 
		//펩시콜라와 오징어 땅콩은 구별해내지 못한다.  //사이다와 조리퐁은 구별해냄

		//문제 1, 배경이 있는 경우에 전체영역을 뽑아내지 못함.
	{
		CMyImage<T> dst(m_nWidth, m_nHeight);
		int count = 0;
		for (int r = 0; r<m_nHeight; r++)
		{
			...
			for (int c = 0; c<m_nWidth; c++)
			{
				...
				if (pSrc[idx + 1] < 100&& pSrc[idx + 2] < 50&& pSrc[idx] > 45)
				{
					count++;
					pDst[c] = 0;  //pepsi콜라 색상이면 검정색
				}
       ...
       /*
			if(count >1700&&count<6000&&(Y_count <1000 || Y_count>18000))
				AfxMessageBox("펩시콜라가 검출 되었습니다.");
	   */
	...
	CMyImage<T> CIDER() const //사이다 부분을 1 ,아닌 부분을 0으로 바이너리 한다. 
	{
		CMyImage<T> dst(m_nWidth, m_nHeight);
		int G_count = 0;
		for (int r = 0; r<m_nHeight; r++)
		{
			...
			for (int c = 0; c<m_nWidth; c++)
			{
				...
				if (pSrc[idx] < 80&& pSrc[idx+1] > 45 && pSrc[idx + 2] < 20)
				{
					G_count++;
					pDst[c] = 0;  //사이다 색상이면 검정색
				}
				idx += 3;
			}
		}
		...
		CString tmpStr;
		tmpStr.Format(_T("%d"), ++G_count);
		
		//AfxMessageBox(tmpStr);
	
		//if(G_count >700)
					//AfxMessageBox("사이다가 검출되었습니다.(By Color)");
	...
	CMyImage<T> JOLLY_PONG() const //조리퐁 부분을 1 ,아닌 부분을 0으로 바이너리 한다. 
	{
		CMyImage<T> dst(m_nWidth, m_nHeight);
		int Y_count = 0;
		for (int r = 0; r<m_nHeight; r++)
		{
			...
			for (int c = 0; c<m_nWidth; c++)
			{
				...
				if (pSrc[idx] < 70 && pSrc[idx + 1] > 60 && pSrc[idx + 2] > 130)
				{
					Y_count++;
					pDst[c] = 0;  //조리퐁 색상이면 검정색
				}
				idx += 3;
			}
		}
		...
		CString tmpStr;
		tmpStr.Format(_T("%d,"), ++Y_count);

		//AfxMessageBox(tmpStr);

		/*
		if(Y_count >18000&&Y_count<22000)
		AfxMessageBox("조리퐁이 검출되었습니다.");
		*/
	...
	CMyImage<T> CUTTLE_PEANUT() const //오징어 땅콩 
							 
	{
		...
		for (int r = 0; r<m_nHeight; r++)
		{
			...
			for (int c = 0; c<m_nWidth; c++)
			{
				...
				{
					B_count++;
					pDst[c] = 0;  //파란 색상이면 검정색
				}

				if ( pSrc[idx] < 80&& pSrc[idx+1] > 60 && pSrc[idx + 2] > 60)
				{
					Y_count++;
					pDst[c] = 127;  //노랑 색상이면 회색
				}
		...
		//printf("%d", count);
		CString tmpStr;
		tmpStr.Format(_T("%d,   %d"), ++B_count, ++Y_count);

		//AfxMessageBox(tmpStr);
		
		/*
		if (B_count >2500 && B_count<7000&& Y_count >2000 && Y_count<3500)
			AfxMessageBox("오징어땅콩이 검출되었습니다.");
		*/
	...
	void Paste(const CMyImage<T>& src, int x, int y)
	{
		ASSERT(src.GetChannel() == m_nChannels);

		int nWidth  = src.GetWidth();
		int nHeight = src.GetHeight();

		for (int r=0 ; r<nHeight ; r++) // 행 단위로 픽셀 데이터 복사
		{
			memcpy(m_pImageData + (y+r)*m_nWStep/sizeof(T) + x*m_nChannels,
				src.GetPtr(r),
				nWidth*m_nChannels);
		}
	}
