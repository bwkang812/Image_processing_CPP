...
void CImageView::SetNumClkPoints(int nPoints) 
{
	m_nMouseClk = nPoints;
}

bool CImageView::GetClkPoints(int* posX, int* posY, int nPoints)
{
	if (m_nMouseClk == 0)
	{
		memcpy(posX, m_nClkPosX, nPoints*sizeof(int));
		memcpy(posY, m_nClkPosY, nPoints*sizeof(int));
		return true;
	}
	else
	{
...
void CImageView::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (m_nMouseClk-- > 0)
	{
		m_nClkPosX[m_nMouseClk] = point.x;
		m_nClkPosY[m_nMouseClk] = m_image.GetHeight()-point.y;
		m_image.GetAt(point.x, point.y) = 255;
		Invalidate();
	}
